...
void EmitAssemblyHelper::FinalizeForJIT() {
  ...
  if (LangOpts.CUDAIsDevice) {
    auto DevMod =
        std::make_unique<llvm::Module>("clang-jit-device",
    ...
    auto NewStrGV = [&](StringRef Data, StringRef Name)
      ...
      GV->setAlignment(decodeMaybeAlign(1));
      ...
      for (auto &I : BB)
        if (auto CS = dyn_cast<CallBase>(&I)) {
          if (auto *Callee = CS->getCalledFunction()) {
            if (auto *FCallee = dyn_cast<llvm::Function>(
                    Callee->stripPointerCasts()))
              if (FCallee->getName() == "__clang_jit")
                JCalls.push_back(CS);
          } else {
            //            llvm::errs() << "Callee is null?\n";
            //            CS->dump();
          }
    ...
    if (auto *DevGV =
      ...
      for (auto* JCS : JCalls) {
        JCS->setArgOperand(10,
                           llvm::ConstantExpr::getPointerBitCastOrAddrSpaceCast(
                               DevGV, JCalls[0]->getArgOperand(10)->getType()));
        JCS->setArgOperand(11, NumDevs);
      }
    }
  }
  ...
  for (auto &JCS : JCalls) {
    JCS->setArgOperand(0, CmdLineStr);
    JCS->setArgOperand(1, CmdLineStrLen);
    JCS->setArgOperand(2, ASTData);
    JCS->setArgOperand(3, ASTDataLen);
    JCS->setArgOperand(4, IRData);
    JCS->setArgOperand(5, IRDataLen);
  }
}

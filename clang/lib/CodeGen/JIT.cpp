...
public:
  ...
  void HandleTranslationUnit(ASTContext &C) override {
    ...
    for (auto &BM : DevLinkMods) {
      ...
      // FIXME: This function has been removed, check out if there is a suitable replacement
//      for (Function &F : *M)
...
struct CompilerData {
  ...
  CompilerData(const void *CmdArgs, unsigned CmdArgsLen,
    ...
    llvm::opt::InputArgList ParsedArgs = driver::getDriverOptTable().ParseArgs(CC1Args, MissingArgIndex, MissingArgCount);
  ...
  void restoreFuncDeclContext(FunctionDecl *FunD) {
    ...
    for (DeclContext *DC : reverse(DeclContextsToReenter)) {
      ...
      S->ActOnReenterTemplateScope(cast<Decl>(DC), [&] {
        return S->getCurScope();
      });

      // We'll reenter the function context itself below.
      if (DC != FunD)
        S->PushDeclContext(S->getCurScope(), DC);
    }
...
//    DeclContext *DD = FunD;
//    DeclContext *NextContaining = DD->getLexicalParent();//S->getContainingDC(DD);
//    while (DD && !DD->isTranslationUnit()) {
//      bool ShouldPush = DD == NextContaining;
//      DeclContextsToReenter.push_back({DD, ShouldPush});
//      if (ShouldPush)
//        NextContaining = DD->getLexicalParent(); //S->getContainingDC(DD);
  ...
  std::string instantiateTemplate(const void *NTTPValues, const char **TypeStrings,
    ...
    for (auto &TA : FTSI->TemplateArguments->asArray()) {
	  ...
	  // function, so local types will remain unavailable.
            ...
            S->CurContext = FunD->getLexicalParent();
          }
    ...
    sema::TemplateDeductionInfo Info(Loc);
    {
      ...
      SMName = std::string(Consumer->getCodeGenerator()->CGM().getMangledName(Specialization));
    }
  ...
  void emitAllNeeded(bool CheckExisting = true) {
    ...
    do {
      ...
      Consumer->getCodeGenerator()->CGM().EmitAllDeferred([&](GlobalDecl GD) {
        ...
        if (!CheckExisting || !CJ->findSymbol(std::string(MName))) {
          Changed = true;
          return false;
        }
      ...
      for (auto &DeclName : DeclNames) {
        if (CheckExisting && CJ->findSymbol(std::string(DeclName)))
          continue;

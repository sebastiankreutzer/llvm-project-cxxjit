...
LoopTransformConfig createRandomLoopConfig(RNETy &RNE) {
  ...
  std::uniform_int_distribution<unsigned> VecWidthDist(
      LoopTransformConfig::MIN_VALS[LoopTransformConfig::VECTORIZE_WIDTH],
  ...
  const unsigned InterleaveMax =
      LoopTransformConfig::MAX_VALS[LoopTransformConfig::INTERLEAVE_COUNT];
  std::uniform_int_distribution<unsigned> InterleaveCountDist(
      LoopTransformConfig::MIN_VALS[LoopTransformConfig::INTERLEAVE_COUNT],
  ...
  Cfg.Vals[LoopTransformConfig::VECTORIZE_PREDICATE_ENABLE] =
      (unsigned)BiasedFlip(90);

  // Probably always beneficial?
  Cfg.Vals[LoopTransformConfig::DISABLE_LICM] = (unsigned)BiasedFlip(10);

  Cfg.Vals[LoopTransformConfig::DISABLE_LICM_VERSIONING] =
      (unsigned)BiasedFlip(30);

  Cfg.Vals[LoopTransformConfig::DISTRIBUTE] = (unsigned)BiasedFlip(50);

  std::uniform_int_distribution<unsigned> UnrollDist(
      LoopTransformConfig::MIN_VALS[LoopTransformConfig::UNROLL_COUNT],

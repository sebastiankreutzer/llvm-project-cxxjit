...
//        KnobState KS(Knobs, Simplex[i].Cfg);
//        KS.dump();
    ...
    case EVAL_REFLECTED: {
      assert(Reflected.Stats && Reflected.Stats->Valid() && "Evaluation not finished");
      if (Reflected.Stats->betterThan(*Simplex.front().Stats)) {
        auto ExpandedVec = Centroid + (Mapping.map(Reflected.Cfg) - Centroid) * P.Gamma;
      ...
      auto ContractedVec = Centroid + (Mapping.map(Simplex.back().Cfg) - Centroid) * P.Rho;
    ...
    case EVAL_EXPANDED: {
      assert(Expanded.Stats && Expanded.Stats->Valid() && "Evaluation not finished");
    ...
    case EVAL_CONTRACTED: {
      assert(Contracted.Stats && Contracted.Stats->Valid() && "Evaluation not finished");
      ...
      for (unsigned i = 1; i < N; i++) {
        auto ShrunkVec = BestVec + (Mapping.map(Simplex[i].Cfg) - BestVec) * P.Sigma;

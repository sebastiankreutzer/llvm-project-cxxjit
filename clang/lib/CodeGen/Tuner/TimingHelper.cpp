...
Function* TimingHelper::createTimingWrapper() {
  ...
  // NOTE: We must save the names here as the module might be modified after having been added to the JIT engine
  ...
  Function* Wrapper = Function::Create(TimedFunction->getFunctionType(), Function::ExternalLinkage, FName, M);
  ...
  instrumentPostCall(IRB, CallCountGlobal, TotalCyclesGlobal, MeanCyclesGlobal, VarNGlobal, CyclesStart);
...
void TimingHelper::instrumentPostCall(IRBuilder<>& IRB, GlobalVariable* CallCount, GlobalVariable* CycleCount, GlobalVariable* MeanCycles, GlobalVariable* VarN, Value* StartCycles) {

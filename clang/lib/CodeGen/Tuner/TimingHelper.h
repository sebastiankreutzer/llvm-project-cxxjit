...
class TimingHelper {
  ...
  void instrumentPostCall(llvm::IRBuilder<>& IRB, llvm::GlobalVariable* CallCount, llvm::GlobalVariable* CycleCount, llvm::GlobalVariable* MeanCycles, llvm::GlobalVariable* VarN, llvm::Value* StartCycles);
...
public:
  ...
  TimingGlobals lookupGlobals(F&& SymLookupFn) {
    TimingGlobals TG;
    TG.CallCount = fetchGlobal<int64_t>(std::forward<F>(SymLookupFn), CallCountGlobalName);
    TG.Cycles = fetchGlobal<int64_t>(std::forward<F>(SymLookupFn), TotalCyclesGlobalName);
    TG.MeanCycles = fetchGlobal<double>(std::forward<F>(SymLookupFn), MeanCyclesGlobalName);

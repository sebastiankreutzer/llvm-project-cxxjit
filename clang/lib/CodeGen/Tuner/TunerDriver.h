...
struct TunedCodeVersion {
  ...
  TunedCodeVersion(VersionID ID, orc::VModuleKey ModKey, void* FPtr, tuner::TimingGlobals Globals, tuner::ConfigEvalRequest Request)
...
struct JITTemplateInstantiation {
  ...
  void updateStats() {
    ...
    // We are interested in maximal performance, so we don't care about the actual distribution of individual timing data
    // within one template specialization.
    // Instead, we view the statistics of the best version as representative of the whole set.
    if (STracker.HasBest)
...
public:
  TunableArgList(llvm::SmallVector<TemplateArgument, 8> BaseArgs) : BaseArgs(std::move(BaseArgs)) {}
...
struct ConfigMapInfo {
  ...
  static bool isEqual(const tuner::KnobConfig &LHS, const tuner::KnobConfig &RHS) {
...
struct TemplateTuningData {
  ...
  llvm::DenseMap<tuner::KnobConfig, JITTemplateInstantiation, ConfigMapInfo> Specializations;

  tuner::KnobConfig ActiveConfig;
  bool Initialized;

  TemplateTuningData(CompilerData& CD, unsigned Idx, std::unique_ptr<tuner::Tuner> TATuner, TunableArgList TunableArgs)
    : CD(CD), Idx(Idx), TATuner(std::move(TATuner)), TunableArgs(std::move(TunableArgs)) {
    Initialized = false;
  }
...
private:
  ...
  JITTemplateInstantiation instantiate(TAList TAs, tuner::ConfigEvalRequest Request) {
//    for (auto& TA: TAs) {
//      TA.dump();
//      errs() << ", ";
//    }
    ...
    auto Opt = llvm::make_unique<tuner::Optimizer>(*CD.Diagnostics,
                                                   *CD.HSOpts,
                                                   CD.Invocation->getCodeGenOpts(),
                                                   *CD.TargetOpts,
                                                   *CD.Invocation->getLangOpts(),
...
private:
  llvm::DenseMap<InstInfo, std::unique_ptr<TemplateTuningData>, InstMapInfo> TuningDataMap;
